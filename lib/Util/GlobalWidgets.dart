...
import 'package:passbook_core/Util/QRScan.dart';
...
class GlobalWidgets {
  ...
  Widget logoWithoutText(
      BuildContext context, TitleDecoration titleDecoration) {
    return Column(
      children: <Widget>[
        SizedBox(
          height: 10.0,
  ...
  showSnackBar(GlobalKey<ScaffoldState> _scaffoldKey, String msg,
    ...
    _scaffoldKey.currentState.showSnackBar(
      SnackBar(
          ...
          duration:
              actions == null ? Duration(seconds: 5) : Duration(seconds: 5),
          action: actions),
    );
  }
  ...
  void shoppingPay(
    ...
    if (shopInfo["Table"][0]["Name"] == 'N') {
      GlobalWidgets().showSnackBar(
          scaffoldKey, "Invalid QR-Code, try to scan our own bank QR-Code");
      return;
    }
      ...
      button: CustomRaisedButton(
        ...
        onPressed: isLoading
            ...
            : () async {
                ...
                Map otpResponse =
                    await RestAPI().get(APis.ownFundTransferOTP(params));
                ...
                if (otpResponse["Table"][0]["Column1"]
                            ...
                            : () async {
                                ...
                                if (_pass != null && _otp != null) {
                                  ...
                                  try {
                                      ...
                                      Navigator.of(context).pushAndRemoveUntil(
                                          MaterialPageRoute(
                                            builder: (context) => Receipt(
                                              ...
                                              transID: response["Table"][0]
                                                      ["TranNO"]
                                                  .toString(),
                                              paidTo: shopInfo["Table"][0]
                                                  ["Name"],
                                              accTo: shopInfo["Table"][0]
                                                      ["AccNo"]
                                                  .toString(),
                                              accFrom: acc,
                                            ),
                                    ...
                                    GlobalWidgets()
                                        .showSnackBar(scaffoldKey, e.message);
                                  }
                                } else {
                  ...
                  Navigator.of(context).pop();
                  GlobalWidgets().showSnackBar(
                      scaffoldKey, ("Something went wrong please try again"));
                }
              },
      ),
    );
...
class EditTextBordered extends StatefulWidget {
  ...
  final Color color, hintColor, borderColor, fillColor, focusColor;
  final int maxLength;
  final int maxLines;
...
class CustomRaisedIndigoButton extends StatefulWidget {
  final bool loadingValue;
  final Function onPressed;
  final String buttonText;
  final double textSize;
  ...
  const CustomRaisedIndigoButton({
    Key key,
    this.loadingValue = false,
    @required this.onPressed,
    @required this.buttonText,
  ...
  @override
  _CustomRaisedIndigoButtonState createState() =>
      _CustomRaisedIndigoButtonState();
}

class _CustomRaisedIndigoButtonState extends State<CustomRaisedIndigoButton> {
  @override
  Widget build(BuildContext context) {
    return RaisedButton(
      color: Theme.of(context).secondaryHeaderColor,
